...
void dec(int num, int i)
{
    for (int a = i * maxnum / 8; a < (i + 1) * maxnum / 8; a++)
    {
        for (int i = 0; i < 4; i = i + 2)
        {
            X[a][i] = secrettext[a][i];
            X[a][i + 1] = secrettext[a][i + 1];
        }
        for (int i = 0; i < 32; i = i + 4)
        {
            X[a][i + 4] = X[a][i] ^ T(X[a][i + 1] ^ X[a][i + 2] ^ X[a][i + 3] ^ rkey[31-i]);
            X[a][i + 5] = X[a][i + 1] ^ T(X[a][i + 2] ^ X[a][i + 3] ^ X[a][i + 4] ^ rkey[30-i]);
            X[a][i + 6] = X[a][i + 2] ^ T(X[a][i + +3] ^ X[a][i + 4] ^ X[a][i + 5] ^ rkey[29-i]);
            X[a][i + 7] = X[a][i + 3] ^ T(X[a][i + 4] ^ X[a][i + 5] ^ X[a][i + 6] ^ rkey[28-i]);
        }
        for (int i = 0; i < 4; i = i + 2)
        {
            plaintext[a][i] = X[a][35 - i];
            plaintext[a][i + 1] = X[a][34 - i];
        }
    }
}
int main()
{
    ...
    for (int k = 0; k < 11; k++) {
        ...
        printf("\nrun_time：%fus\n", run_time);
        double result = 16 * maxnum * 1000 / run_time;
        printf("吞吐量为%f字节/ms", result);
    }
}
